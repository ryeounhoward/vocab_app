...
class _SearchPageState extends State<SearchPage> {
  ...
  void _onSearch(String query) async {
    ...
    if (groupId != null) {
      allowedIds = await dbHelper.getWordIdsForGroup(groupId);
      filterActive = true;
    ...
    final filteredWords = words
        .where((item) {
          ...
          return item['word'].toString().toLowerCase().contains(normalized);
        })
        .map((e) {
          final int id = e['id'] as int? ?? -1;
          final bool isInGroup =
              !filterActive || (allowedIds?.contains(id) ?? false);
          return {...e, 'table': DBHelper.tableVocab, '_isInGroup': isInGroup};
        })
        .where(
          (item) =>
    ...
    final Set<int> resultIds = filteredWords
        .map((e) => e['id'])
        .whereType<int>()
        .where((id) => id > 0)
        .toSet();
    final groupsByWordId = await dbHelper.getWordGroupsForWordIds(resultIds);

    final List<Map<String, dynamic>> withGroups = filteredWords.map((e) {
      final int id = e['id'] as int? ?? -1;
      return {
        ...e,
        '_groups': groupsByWordId[id] ?? const <Map<String, dynamic>>[],
      };
    }).toList();
